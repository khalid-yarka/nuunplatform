// ============================================
// UPGRADE SHEET – Feature & Upgrade Modals
...
(function() {
    ...
    // Prices use canonical tier keys: premium / pro
    const PRICES = {
    ...
    const SAVINGS = {
        premium: { monthly: 0, term: 40, yearly: 66 },
        pro:     { monthly: 0, term: 43, yearly: 71 },
    };
    ...
    function init() {
        ...
        // Backward-compat aliases (removed in Phase 6)
        window.openSafkaPreview = openUpgradeSheet;
        window.closeSafkaSheet = closeSheet;

        window.openUpgradeSheetFromTier = function(tier) {
            openUpgradeSheet({ requiredTier: tier });
        };
    ...
    function openUpgradeSheet(options) {
        ...
        // Normalize legacy tier input
        let tier = options.requiredTier || null;
        if (tier === 'danbe') tier = 'free';
        else if (tier === 'dhexe') tier = 'premium';
        else if (tier === 'hore') tier = 'pro';
        // If 'free' is passed, we still open — user will pick a plan
