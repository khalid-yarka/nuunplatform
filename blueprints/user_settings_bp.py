...
from db import (
    get_student_by_id, is_admin, execute_with_retry,
    get_student_by_public_id, get_user_subject_list,
)
from user_settings import (
    get_user_settings, update_user_settings, get_user_setting,
)
from services.tier_service import (
    get_feature_level, get_current_user_tier,
    can_create_live_quiz, has_feature,
)
...
def update_profile():
    ...
    log_activity('user.profile_update',
                 f"User {user_id} updated profile", 'info', user_id=user_id)
...
def update_public_id():
    ...
    if tier == 'free':
        return jsonify({'error': 'Public ID management is not available for your tier. Upgrade to Premium or Pro.'}), 403

    if tier == 'premium':
        if action != 'regenerate':
            return jsonify({'error': 'Premium tier can only regenerate a new random ID.'}), 400
        attempts = 0
        while attempts < 10:
            candidate = ''.join(
    ...
    log_activity('user.public_id_update',
                 f"User {user_id} updated public ID to {new_id}",
...
def update_appearance():
    ...
    log_activity('user.theme_update',
                 f"User {user_id} changed appearance", 'info', user_id=user_id)
...
def update_quiz_preferences():
    ...
    if auto_skip and not has_feature("quiz_auto_advance", user_id):
    ...
    log_activity('user.quiz_preferences_update',
                 f"User {user_id} updated quiz preferences", 'info', user_id=user_id)
...
def update_notifications():
    ...
    if updates:
        update_user_settings(user_id, updates)
        log_activity('user.notification_preferences_update',
                     f"User {user_id} updated notification preferences",
                     'info', user_id=user_id)
...
def update_privacy():
    ...
    if updates:
        update_user_settings(user_id, updates)
        log_activity('user.privacy_update',
                     f"User {user_id} updated privacy settings",
                     'info', user_id=user_id)
...
def change_password():
    ...
    student = get_student_by_id(user_id)
    if not student or not check_password_hash(student['password'], current):
    ...
    log_activity('user.password_change',
                 f"User {user_id} changed password", 'warning', user_id=user_id)
